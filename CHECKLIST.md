...
- [x] **데모 영상 섹션에 라벨 추가 + "The app" 2컬럼 배치, No. 순서 정정, 홈 H1 한 줄 처리**
  (2026-08-13) — 세 가지를 한 번에 처리.
  - **데모 영상이 "뚝 나타난다"는 피드백**: 다른 섹션은 전부 mono 라벨 + `border-t` 구분선이
    있는데 데모 영상 섹션만 라벨도 구분선도 없이 좁은 폰 화면 하나만 덩그러니 떠 있었던 게 원인.
    `Demo` 타입에 `label`("The app")과 `caption`(영상 내용 요약 문장) 필드 추가, 다른 섹션들과
    동일하게 라벨+구분선을 얹고, 세로 영상일 때는 히어로/The name과 같은
    `lg:grid-cols-[minmax(0,360px)_1fr]` 2컬럼(영상 왼쪽, 캡션 오른쪽)으로 배치해서 화면 폭을
    맥락 있게 채우도록 함.
  - **No. 순서 오류**: Roun이 No.002, Wordflow가 No.001로 반대로 되어 있던 걸 본인이 지적. `apps.ts`
    에서 Roun → 001, Wordflow → 002로 정정(배열 순서도 이미 Roun이 먼저라 카드 표시 순서와 일치).
  - **홈페이지 H1 줄바꿈**: "I build the things I need first."가 `max-w-3xl` 제약 때문에 "first."
...
- [x] **히어로에 이름 설명 티저 링크 추가** (2026-08-13) — 와이프 피드백: Roun이 실제 아들 이름과
  ...
  필요.
- [x] **티저 링크에 "full story below" 문구 추가 + 폭 제한으로 캐치라인 공간 침범 방지**
  (2026-08-13) — 배포 확인 후 본인이 바로 이어서 피드백 두 개.
  1. 티저 링크가 그냥 뜻만 나열해서 "누르면 뭐가 나오는지" 불명확했다는 지적으로,
     "· full story below ↓" 추가.
  2. 그런데 이 문구가 길어지면서 히어로의 `lg:grid-cols-[auto_1fr]` 왼쪽 컬럼(`auto` 폭, 아이콘+
     제목+티저로 폭이 결정됨)이 텍스트 길이만큼 넓어져서, 오른쪽 컬럼(캐치라인 "Save the moments
     you'll want to hand back to them, someday.", 더 중요한 카피)이 좁아지는 문제 발생. 티저
     링크에 `max-w-[220px]`를 줘서 3줄로 자연스럽게 줄바꿈되게 하고, 왼쪽 컬럼 폭이 아이콘+
     "Roun" 제목 크기 이상으로 안 커지게 고정. 오른쪽 캐치라인은 원래 공간 그대로 유지.
  - 이 작업 중간에 시스템 메모리 고갈 문제(옆에서 실제 Roun 앱 Capacitor 빌드 동시 실행 + 5일째
    방치된 좀비 `claude continue` 프로세스 PID 53892가 CPU 계속 점유)로 `next build`/`next dev`가
...
- [x] **"Say hi, get extra time" 추천/친분 트라이얼 연장 카드 추가** (2026-08-13) — 본인 아이디어:
  단순 서포트/피드백/버그 리포트 용도뿐 아니라 그냥 인사하러 이메일해도 된다는 걸 알려주고,
  아는 사람이 인사하면 3개월 무료, 모르는 사람이 연락하면(콜드 아웃리치) 트라이얼에 1개월
  추가해주는 정책을 카드로 안내. Pricing 섹션 바로 아래(트라이얼 얘기가 이미 나온 문맥이라
  자연스러움)에 틴트 박스로 배치. `apps.ts`에 `Referral` 타입(`title`, `body`, `email`) 추가.
  본문 카피에 "Emailing me isn't just for bugs or questions"로 시작해서 서포트 용도가 아닌
  이메일도 환영한다는 걸 명시. 액션 링크 두 개: `mailto:support@sl-studio.dev` 직접 이메일,
...
- [x] **"The app" 영상 섹션을 클릭형 스크린샷 캐러셀로 교체** (2026-08-13) — 본인이 레포에
  `roun-howto-v3.mp4`(500x970, 41.8초, 실제 앱 화면 녹화 + 화면마다 하단에 캡션 자막)를 넣어둠.
  ffmpeg로 1초 간격 프레임 추출해서 캡션이 바뀌는 지점을 전부 수동으로 확인(처음엔 4초 간격일
  거라 추측했는데 실제로는 구간마다 길이가 다 달라서, 2초/1초 간격으로 재스캔해서 정확한 경계
  찾음), 총 12개의 서로 다른 화면/캡션 구간 확인(처음엔 9~10개로 추정했었는데 실제로는 Feed 인트로,
  마일스톤 상세 보기 등 빠뜨린 구간이 있었음). 각 구간마다 전환 애니메이션 없는 깨끗한 프레임 골라서
  캡션 자막 바(하단 어두운 배너) 픽셀 좌표 감지해서 크롭 제거, `public/roun/howto/howto-01.jpg`
  ~`howto-12.jpg`로 저장(500x828, JPEG).
  - 본인 요청: "화면 하나하나씩 순서대로 스크린샷으로 떠서... 그 영상 넣어놨던 자리를 이걸로
    대체... 화살표 앞뒤로 1/9, 2/9 이렇게 몇 번째 사진인지 보여주면서... 현재 왼쪽에 화면, 오른쪽에
    코멘트 유지하면서... 영상 속 캡션을 더 elaborate한 말로 적어놓으면 됨". 요청대로 기존
    좌(화면)/우(코멘트) 레이아웃 유지, 화살표 클릭으로 넘기는 인터랙티브 캐러셀 구현, "N / 12"
    인디케이터, 영상 속 짧은 자막(예: "Filter by kid or pet") 대신 왜 유용한지까지 설명하는 긴
    문장으로 12개 전부 새로 작성(예: "Only want to see one kid's or pet's entries? Filter the
    Feed down to just them with a tap.").
  - `src/components/how-to-carousel.tsx`(신규, `"use client"`, `useState`로 인덱스 관리, 좌우
    화살표 버튼, 순환(마지막에서 다음 누르면 1번으로)) 구현. `apps.ts`에 `HowToStep`/`HowTo` 타입
    추가, Roun의 `demo` 필드를 `howTo`로 교체(라벨 "The app" 그대로 유지, 같은 자리에 렌더링).
  - **정리**: 이제 안 쓰는 `Demo` 타입, `demo?: Demo` 필드, `src/components/demo-media.tsx`
    컴포넌트 완전히 삭제(참조하는 곳이 하나도 안 남아서). 다만 실제 mp4 파일(`public/demos/
    roun-demo.mp4`, `roun-demo-full.mp4`)은 나중에 다른 용도(유튜브 등)로 쓸 수도 있어서 그대로
    남겨둠, 페이지에서만 안 씀.
  - `next build` 통과, 로컬 브라우저에서 화살표 클릭 테스트(다음/다음.../마지막 12/12에서 한 번 더
    누르면 1/12로 순환) 전부 정상 동작 확인.
- [x] **스크린샷에서 캡션 배너 크롭 제거(본인이 "자를 필요 없다"고 지적)** (2026-08-13) — 배포 직후
  본인이 "스크린샷에 밑에 달아논 코멘트를 다 잘랐네? 그거 자를 필요 없는데"라고 지적. 원래 의도를
  다시 확인해보니, 영상 속 짧은 자막(캡션 배너)은 스크린샷 안에 그대로 남겨두고, 그 옆(오른쪽)에
  내가 쓴 elaborate한 설명을 추가로 붙이는 거였음(둘 다 보이게, 자막 제거가 아니라). 문제는 이
  작업을 하려면 원본 영상이 필요한데, 이전 정리 과정에서 `roun-howto-v3.mp4`를 실수로 삭제해버려서
  본인한테 다시 넣어달라고 요청, 본인이 다시 넣어줌. 12개 스크린샷 전부 캡션 배너 크롭 없이 원본
  풀프레임(500x970)으로 재추출해서 `public/roun/howto/howto-0X.jpg` 교체, `apps.ts`의 `height`
  값도 828 → 970으로 전부 갱신.
  - **버그 발견**: 파일을 같은 경로로 덮어썼는데도 브라우저에 캡션 배너가 안 보여서 확인해보니
    Next.js Image 최적화 캐시(`_next/image` 엔드포인트)가 이전 크롭 버전(500x828)을 계속 서빙하고
    있었음(`naturalHeight` 확인해보니 정확히 이전 크롭 높이의 절반이었음, 즉 캐시된 최적화본).
    `curl`로 raw 정적 파일 자체는 이미 새 파일(500x970)로 확인됐는데 Next 최적화 레이어만 stale.
    `.next` 전체 삭제 + dev 서버 완전 재시작으로 해결(단순 `.next/cache/images`만 지우는 걸로는
    안 됨, 서버가 메모리에 들고 있었던 듯). `next build` 통과, 로컬 브라우저에서 캡션 배너 포함해서
    정상 렌더링 확인.
- [x] **How-to 캐러셀 스크린샷 크기 확대(데스크톱/모바일 둘 다 너무 작다는 피드백)** (2026-08-13)
  — 본인 피드백: "스크린샷 화면이 컴퓨터로 보니까 너무 작다", "핸드폰으로는 구분이 안 되는데".
  기존엔 화살표 버튼이 이미지 좌우에 나란히(flex row) 배치돼서 가로 공간을 잡아먹는 바람에
  이미지 자체가 `max-w-[280px]/[300px]`로 작았던 게 원인. 화살표를 이미지 레이아웃 흐름에서
  빼서 이미지 위에 겹쳐지는 절대 위치(`absolute`, 좌우 가장자리, 반투명 배경 + blur)로 변경,
  이미지가 가로 공간을 훨씬 더 많이 쓸 수 있게 함(`max-w-[360px] sm:max-w-[400px]`로 확대,
  거의 30~40% 더 큼). 오른쪽 텍스트 컬럼에 더 공간 주려고 그리드도 `minmax(0,360px)` →
  `minmax(0,440px)`로 확장. `next build` 통과, 로컬 스크린샷으로 확대되고 화살표가 이미지 위에
  자연스럽게 떠 있는 것 확인.
- [x] **화살표를 이미지 위에 겹치지 않고 양옆 바깥으로 이동** (2026-08-13) — 바로 다음 피드백:
  "양옆 화살표가 스크린샷 안에 들어오는데. 화살표는 스크린샷 양옆 바깥에 있는게 좋아". 방금 만든
  절대 위치(이미지 위에 겹침) 방식을 되돌리고, 원래의 flex row 방식으로 복귀하되 이번엔 그리드
  컬럼을 `minmax(0,520px)`로 더 넓혀서 화살표(각 40px)+간격+이미지(`max-w-[340px]/[380px]`)가
  전부 한 줄에 여유 있게 들어가게 조정. 결과적으로 이미지도 이전 겹침 버전과 비슷하게 크면서
  화살표는 완전히 이미지 바깥 좌우에 위치. `next build` 통과, 로컬 브라우저에서 화살표 클릭
  동작 및 레이아웃 확인.
- [x] **Vercel Web Analytics 추가 (방문자 수/조회수 비공개로 확인)** (2026-08-14) — 본인 요청:
  방문자 수/조회수를 나만 볼 수 있게(퍼블릭 노출 없이) 확인하고 싶다. `@vercel/analytics` 패키지
  설치, 루트 레이아웃(`src/app/layout.tsx`)에 `<Analytics />` 추가(사이트에는 아무 UI도 안 보임,
  백그라운드에서 방문 데이터만 수집). 데이터는 Vercel 대시보드의 이 프로젝트 Analytics 탭에서만
  보임(본인 Vercel 계정으로 로그인해야 접근 가능, 완전 비공개). `next build` 통과. **참고**:
  Analytics 탭 활성화는 코드/CLI로 안 되고 대시보드에서 한 번 켜야 함(무료 Hobby 플랜에서도 기본
  제공되는 걸로 확인, 월 데이터 포인트 제한 있음) — 배포 후 본인이 대시보드에서 직접 켜야 함.
